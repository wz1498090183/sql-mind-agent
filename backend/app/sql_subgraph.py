...
def _references_undefined_upstream(sql: str, upstream_results: dict | None) -> list[str]:
    """检测 SQL 是否把上游子任务 id 误当作真实表名引用。

    LLM 生成依赖型 SQL 时常见错误：把上游子任务 id（如 t1）直接写进
    FROM/JOIN，当作表来引用，而 SQLite 里并不存在名为 t1 的表。
    若 SQL 中没有定义同名 CTE（WITH t1 AS ...），则该 id 就是误用。

    实现：
        1. 提取所有 WITH 子句里定义的 CTE 名（大小写不敏感）。
        2. 正则捕获 FROM/JOIN 后的首个表名。
        3. 表名同时命中「上游 id 集合」且「未在 CTE 定义中」→ 判定为误用。

    Args:
        sql: 待检查的 SQL 语句。
        upstream_results: 上游依赖结果映射 {dep_id: result}，键即上游子任务 id。

    Returns:
        list[str]: 被误用的上游 id 列表（去重、保持出现顺序），空列表表示无此类问题。
    """
    if not upstream_results:
        return []

    upstream_ids = {str(uid).upper() for uid in upstream_results.keys()}

    # 1. 提取 WITH 子句里定义的 CTE 名（大小写不敏感，排除 AS 后的子查询体）
    cte_names: set[str] = set()
    for match in re.finditer(r"\bWITH\b(.+?)\bSELECT\b", sql, re.IGNORECASE | re.DOTALL):
        header = match.group(1)
        for m in re.finditer(r"([A-Za-z_][A-Za-z0-9_]*)\s+AS\s*\(", header, re.IGNORECASE):
    ...
    # 3. 命中上游 id 且未在 CTE 定义中 → 误用
    result: list[str] = []
    seen: set[str] = set()
    for name in matches:
        upper = name.upper()
        if upper in upstream_ids and upper not in cte_names and upper not in seen:
            seen.add(upper)
            result.append(name)
    return result
...
- 依赖上游子任务结果时，用 WITH ... AS (...) 定义同名 CTE 后引用，
  或在 FROM/JOIN 里写内联子查询；**绝不可把上游子任务 id（如 t1）直接当作表名引用**"""
...
def _generate_sql_node(state: SubGraphState) -> dict:
    ...
    # 格式化上游结果：把 {columns, rows} 渲染成 Markdown 表格，便于 LLM 理解列名
    upstream_text = ""
    if upstream_results:
        chunks: list[str] = []
        for k, v in upstream_results.items():
            if isinstance(v, dict) and v.get("success"):
                cols = v.get("columns", [])
                rows = v.get("rows", [])
                lines = [f"### {k}", "", "| " + " | ".join(cols) + " |", "| " + " | ".join(["---"] * len(cols)) + " |"]
                for row in rows[:20]:  # 最多展示 20 行，避免 Prompt 过长
...
def _validate_sql_node(state: SubGraphState) -> dict:
    ...
    执行四层校验：
        ...
        4. 上游子任务 id 误用校验 — 检测依赖型 SQL 是否把上游 id（如 t1）
           误当作表名引用且未定义同名 CTE。
    ...
    if not errors:
        ...
        # 而 SQL 里又没有定义同名 CTE。此时 SQLite 会报 "no such table: t1"。
        misused = _references_undefined_upstream(sql, upstream_results)
        if misused:
            errors.append(
                f"SQL 将上游子任务 id {misused} 误当作表名引用，"
                f"但未定义同名 CTE。请改用 WITH {misused[0]} AS (...) 定义 CTE 后再引用，"
                f"或在 FROM/JOIN 中写内联子查询。"
            )
