...
def get_connection(db_id: str) -> sqlite3.Connection:
    ...
    # 相对路径基于仓库根目录解析（spider_data 在仓库根，而非 backend/ 下）
    db_path = (Path(root) if Path(root).is_absolute() else _project_root.parent / root).resolve()
...
if __name__ == "__main__":
    ...
    if not db_path.is_file():
        print(
            f"数据库 {db_id!r} 在 {db_path.resolve()} 不存在，无法自测",
            file=sys.stderr,
        )
        sys.exit(1)
